...
class MultiHeadAttention(nn.Module):
    ...
    def forward(self, x, use_cache=False):
        ...
        ### updated
        if x.dim() == 2:
            # 如果是 [M, D]，我们假设 M 就是 num_tokens (batch 默认为 1)
            # 或者根据你的需求，认为 M 是 batch*seq
            num_tokens, d_in = x.shape
            b = 1 
            # 为了后续 view 操作方便，临时转回 3D
            #x = x.unsqueeze(0) 
        else:
...
class GPTModel(nn.Module):
    ...
    def forward(self, in_idx, use_cache=False):
        ...
        # change batch matmul to matmul
        # --- 关键修改：展平为 2D (M, K) ---
        # 将 [batch, seq, emb_dim] 变为 [batch * seq, emb_dim]
        # only change the 1 * 1 * n
        # if batch_size == 1 and seq_len == 1:
        #     x = x.view(1, -1) # 变成 [1, 768]
        # else:
        #     pass
        x = x.view(-1, x.size(-1))
...
def load_weights_from_hf(custom_model, name):
    ...
    # manual positional embedding resize
...
def get(name,layer):
    ...
    out = generate_text_simple(
        ...
        idx=encoded_tensor,
        max_new_tokens=10,
        context_size=4096#get_config_from_hf(PRETRAINED_NAME)["context_length"]
    )
