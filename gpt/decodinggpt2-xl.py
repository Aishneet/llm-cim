...
# ========== User parameters ==========
PRETRAINED_NAME = "gpt2-xl"
NEW_NUM_LAYERS = 1
EXPORT_DECODE_PATH = "Decode_linalg2_gpt2_xl_4096.mlir"
DEVICE = "cpu"
...
class MultiHeadAttention(nn.Module):
    ...
    def forward(self, x, use_cache=False):
        ...
        # keep the 2D shape for export friendliness
...
def load_weights_from_hf(custom_model, name):
    ...
    # extend position embeddings from 1024 to 2048
...
def export_decoding_mlir(model, tokenizer, export_path, device="cpu"):
    ...
    class DecodingWrapper(torch.nn.Module):
        ...
        def forward(self, input_ids):
            return self.m(input_ids, use_cache=True)

    wrapper = DecodingWrapper(model)

    decoding_input = torch.zeros((1, 1), dtype=torch.long, device=device)

    prefill_len = model.pos_emb.num_embeddings - 1  # 2047 for context_length=2048
